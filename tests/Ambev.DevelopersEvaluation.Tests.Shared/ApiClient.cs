...
public class ApiClient
{
    ...
    public async Task<HttpResponseMessage> PutAsync(string route, object payload, string? accessToken = null)
    {
        var requestContent = new StringContent(JsonConvert.SerializeObject(payload), Encoding.UTF8, "application/json");
        ...
        return await _httpClient.PutAsync(route, requestContent);
    }
    
    public async Task<HttpResponseMessage> DeleteAsync(string route, string? accessToken = null)
    {
        if (!string.IsNullOrWhiteSpace(accessToken))
        {
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);
